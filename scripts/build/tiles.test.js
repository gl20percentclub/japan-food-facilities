...
// generateTiles は書き出しが非同期のため、この1件だけ async で検証する
await (async function testGenerateTiles() {
  const name = 'generateTiles: metadata.json と、各点に対応する非空 pbf タイルを生成する';
  const dir = tmpDir();
...
await (async function testGenerateTilesThinning() {
  const name = 'generateTiles: 低ズームは間引かれ、最大ズームは全点そのまま';
  const dir = tmpDir();
  const outDir = path.join(dir, 'tiles');
  try {
    // 東京駅周辺の 0.0005 度刻み（約 50m）の 400 点。z6 では同じセルに集まり、
    // z12 ではセルが十分細かいので散らばる。
    const facilities = [];
    for (let i = 0; i < 20; i++) {
      for (let j = 0; j < 20; j++) {
        facilities.push({
    ...
    assert.ok(
      zoomBytes(6) * 4 < zoomBytes(12),
      `z6(${zoomBytes(6)}B) が z12(${zoomBytes(12)}B) より十分小さい`,
    );
...
await (async function testGzipOutput() {
  const name = 'generateTiles: タイルを gzip 圧縮して書き出す（解凍すると MVT に戻る）';
  const dir = tmpDir();
  const outDir = path.join(dir, 'tiles');
  try {
    ...
    const [x, y] = lonLatToTile(139.737, 35.673, 12);
    const pbfPath = path.join(outDir, '12', String(x), `${y}.pbf`);
    const stored = fs.readFileSync(pbfPath);
...
// 既定値をモジュール定数に固定していると、maxZoom だけを指定した呼び出しで
// 最大ズームまで間引かれ、overzoom（z13 以上）の元データが代表点だけになる。
await (async function testDetailZoomFollowsMaxZoom() {
  const name = 'generateTiles: detailZoom 未指定なら maxZoom が間引き対象から外れる';
  const dir = tmpDir();
  const outDir = path.join(dir, 'tiles');
  try {
    const facilities = [];
    ...
    assert.ok(
      zoomBytes(9) * 4 < zoomBytes(10),
      `z9(${zoomBytes(9)}B) が z10(${zoomBytes(10)}B) より十分小さい＝z10 は間引かれていない`,
    );
    passed++;
    console.log(`  ✓ ${name}`);
  } finally {
...
await (async function testDeterministicOutput() {
  const name = 'generateTiles: 同じ入力なら同じバイト列を書き出す（gzip に時刻を埋めない）';
...
console.log(`\n✅ build/tiles ユニットテスト: ${passed}件すべて合格`);
